...
const OUT_FILE = '/home/matt/.openclaw/workspace/TECH/news_raw_0803.json';
const CONTENT_FILE = '/home/matt/.openclaw/workspace/TECH/news_content_0803.json';
...
const AI_KEYWORDS = [
  ...
  'sam altman', 'dario amodei', 'jeff dean', '黄仁勋', 'jensen huang',
...
async function main() {
  ...
  // Filter Aug 1-3 (allow slight buffer)
  ...
  process.stderr.write(`\nAI articles from Aug 1-3: ${deduped.length}\n`);

  // Also grab July 31 as backup
  const candidates31 = Array.from(seen.values()).filter(a => {
    if (!a.link.includes('/2026/')) return false;
    const m = a.link.match(/\/2026\/(\d+)\/(\d+)\//);
    if (!m) return false;
    if (parseInt(m[1]) !== 7) return false;
    if (parseInt(m[2]) !== 31) return false;
    return isAIRelated(a.title);
  });
  process.stderr.write(`AI articles from July 31 (backup): ${candidates31.length}\n`);

  const allToFetch = [...deduped, ...candidates31].slice(0, 35);
  process.stderr.write(`Fetching content for ${allToFetch.length} articles...\n`);
