...
const OUT_FILE = '/home/matt/.openclaw/workspace/TECH/news_raw_0728.json';
const CONTENT_FILE = '/home/matt/.openclaw/workspace/TECH/news_content_0728.json';
...
async function main() {
  ...
  // Step 1: Collect article lists from all sources
  ...
  process.stderr.write(`\nAI articles from July 27-28: ${deduped.length}\n`);

  // Also check for any from 07/26 that may be relevant
  const candidates26 = Array.from(seen.values()).filter(a => {
    if (!a.link.includes('/2026/')) return false;
    if (!a.link.includes('/07/')) return false;
    const m = a.link.match(/\/2026\/07\/(\d+)\//);
    if (!m) return false;
  ...
  process.stderr.write(`AI articles from July 26 (backup): ${candidates26.length}\n`);

  // Step 2: Fetch content for top articles
  const allToFetch = [...deduped, ...candidates26].slice(0, 25);
  const results = [];
