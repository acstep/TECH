...
const AI_KEYWORDS = [
  ...
  'agent', 'model', 'voice', 'image generation', 'stable diffusion',
  'techcrunch', 'startup', 'funding', 'valuation', 'billion', 'million',
  'robotics', 'autonomous', 'llama', 'mistral', 'copilot', 'baseten'
];
...
async function main() {
  ...
  // Filter to July 2026 and AI-related
  ...
  process.stderr.write(`\nTotal AI articles from July 2026: ${deduped.length}\n`);
  
  // Fetch content for all deduped articles (up to 30)
  const topArticles = deduped.slice(0, 30);
  const results = [];
