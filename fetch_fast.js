...
async function main() {
  ...
  process.stderr.write(`\nTotal AI articles July 10-15: ${candidates.length}\n`);
  
  // Fetch content for all articles
  const results = [];
  for (let i = 0; i < candidates.length; i++) {
    const art = candidates[i];
    process.stderr.write(`[${i+1}/${candidates.length}] ${art.title.substring(0, 50)}...\n`);
    const r = await fetchArticleContent(art.link);
    r.title = art.title; // use original title
    results.push(r);
    if (r.success) {
    ...
    if ((i + 1) % 5 === 0) {
      fs.writeFileSync('/tmp/tc_news_partial.json', JSON.stringify(results, null, 2));
      process.stderr.write(`  [Saved ${i+1} articles]\n`);
    }
    await new Promise(r => setTimeout(r, 1200));
  }
  
  fs.writeFileSync('/tmp/tc_news_full.json', JSON.stringify(results, null, 2));
  process.stderr.write(`\nDone! Saved ${results.length} articles to /tmp/tc_news_full.json\n`);
