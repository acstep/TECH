...
const OUT_FILE = '/home/matt/.openclaw/workspace/TECH/news_raw_0729.json';
const CONTENT_FILE = '/home/matt/.openclaw/workspace/TECH/news_content_0729.json';
...
async function main() {
  ...
  // Filter to July 28-29 AI-related (today and yesterday)
  ...
  process.stderr.write(`\nAI articles from July 28-29: ${deduped.length}\n`);

  // Also check for any from 07/27 that may be relevant
  const candidates27 = Array.from(seen.values()).filter(a => {
    if (!a.link.includes('/2026/')) return false;
    if (!a.link.includes('/07/')) return false;
    const m = a.link.match(/\/2026\/07\/(\d+)\//);
    if (!m) return false;
  ...
  process.stderr.write(`AI articles from July 27 (backup): ${candidates27.length}\n`);

  // Step 2: Fetch content for top articles
  const allToFetch = [...deduped, ...candidates27].slice(0, 25);
  const results = [];
