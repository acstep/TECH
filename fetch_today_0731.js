...
const OUT_FILE = '/home/matt/.openclaw/workspace/TECH/news_raw_0731.json';
const CONTENT_FILE = '/home/matt/.openclaw/workspace/TECH/news_content_0731.json';
...
async function main() {
  ...
  // Filter July 30-31 (yesterday and today)
  const candidates = Array.from(seen.values()).filter(a => {
    ...
    if (day < 30) return false; // July 30 or 31
  ...
  process.stderr.write(`\nAI articles from July 30-31: ${deduped.length}\n`);

  // Also grab July 29 as backup
  const candidates29 = Array.from(seen.values()).filter(a => {
    if (!a.link.includes('/2026/')) return false;
    const m = a.link.match(/\/2026\/(\d+)\/(\d+)\//);
    if (!m) return false;
    if (parseInt(m[1]) !== 7) return false;
    if (parseInt(m[2]) !== 29) return false;
    return isAIRelated(a.title);
  });
  process.stderr.write(`AI articles from July 29 (backup): ${candidates29.length}\n`);

  const allToFetch = [...deduped, ...candidates29].slice(0, 35);
  const results = [];
