...
const OUT_FILE = '/home/matt/.openclaw/workspace/TECH/news_raw_0804.json';
const CONTENT_FILE = '/home/matt/.openclaw/workspace/TECH/news_content_0804.json';
...
const AI_KEYWORDS = [
  ...
  'amd', 'intel', 'qualcomm', 'deepfake', 'ai agent', 'reasoning',
...
async function main() {
  console.log('Starting TechCrunch AI news fetch for 2026-08-04...');
  
  // Check if partial results exist
  let partialArticles = [];
  let partialContents = [];
  if (fs.existsSync(OUT_FILE)) {
    partialArticles = JSON.parse(fs.readFileSync(OUT_FILE));
    console.log(`Loaded ${partialArticles.length} partial articles`);
  }
  if (fs.existsSync(CONTENT_FILE)) {
    partialContents = JSON.parse(fs.readFileSync(CONTENT_FILE));
    console.log(`Loaded ${partialContents.length} partial contents`);
  }
  ...
  // Fetch all sources for article lists
  ...
  const filtered = allArticles.filter(a => {
    ...
    if (a.time) {
      ...
      if (now - artTime > oneDay * 5) return false;
    }
    return true;
  });

  console.log(`AI-related articles: ${filtered.length}`);

  // Fetch content only for articles without content yet
  const fetchedLinks = new Set(partialContents.map(c => c.link));
  const toFetch = filtered.filter(a => !fetchedLinks.has(a.link)).slice(0, 30);
