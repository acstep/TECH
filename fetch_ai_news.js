...
const AI_KEYWORDS = [
  ...
  'agent', 'model', 'voice', 'image generation', 'text to', 'stable diffusion',
...
async function main() {
  ...
  // Step 1: Collect article list from all sources
  ...
  // Filter to AI-related articles (2026-07 only)
  const today = '2026-07';
  ...
  process.stderr.write(`\nTotal AI articles from today: ${deduped.length}\n`);
  
  // Step 2: Fetch content for top articles (max 25)
  const topArticles = deduped.slice(0, 25);
  const results = [];
  
  for (const art of topArticles) {
    process.stderr.write(`Fetching content: ${art.title.substring(0, 50)}...\n`);
